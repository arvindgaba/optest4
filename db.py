...
# Handle optional streamlit_autorefresh import
try:
    from streamlit_autorefresh import st_autorefresh
    AUTOREFRESH_AVAILABLE = True
    log.info("streamlit_autorefresh available - autorefresh enabled")
except ImportError:
    AUTOREFRESH_AVAILABLE = False
    log.warning("streamlit_autorefresh not available - autorefresh disabled")
    def st_autorefresh(*args, **kwargs):
        """Dummy function when autorefresh is not available"""
        return None

# Log plotly availability status
if PLOTLY_AVAILABLE:
    log.info("plotly available - interactive charts enabled")
else:
    log.warning("plotly not available - using fallback data tables")
...
# Auto-refresh if available, otherwise show manual refresh option
if AUTOREFRESH_AVAILABLE:
    st_autorefresh(interval=AUTOREFRESH_MS, key="nifty_autorefresh")
else:
    # Add manual refresh button when autorefresh is not available
    if st.button("🔄 Refresh Data", help="Manual refresh (auto-refresh not available)"):
        st.rerun()
...
if df_trend.empty:
    ...
    if df_trend.empty:
        ...
        # Build figure - conditional on plotly availability
        if PLOTLY_AVAILABLE:
            fig_trend = px.line(
                df_plot,
                x="Time",
            ...
            # Fallback when plotly is not available - show data table instead
            st.warning("📊 Interactive chart not available (plotly not installed). Showing data table instead:")
            st.dataframe(df_plot.set_index("Time"), use_container_width=True)
